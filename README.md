# Resumo Gerencial Diário — Power BI → E-mail
...
Uma parte da configuração acima é só credencial/infraestrutura (variáveis de ambiente, secrets, agendamento) — funciona igual pra qualquer dataset. **A outra parte é específica do modelo de dados** e precisa ser editada à mão, porque o script foi escrito em cima de nomes reais de tabelas, colunas e medidas do modelo original. Um fork "puro" (só trocando as variáveis de ambiente) **não vai funcionar** até você fazer esses ajustes.
